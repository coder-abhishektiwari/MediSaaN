...
export function addMedicine(data: any) {
  db.execute(
    `INSERT INTO medicines (patient_id, name, generic_name, dose_amount, dose_unit, times_per_day, dose_times, days_type, custom_days, start_date, end_date, stock_quantity, notes)
     VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)`,
    [data.patient_id, data.name, data.generic_name, data.dose_amount, data.dose_unit, data.times_per_day, JSON.stringify(data.dose_times), data.days_type, JSON.stringify(data.custom_days), data.start_date, data.end_date, data.stock_quantity, data.notes]
  );
